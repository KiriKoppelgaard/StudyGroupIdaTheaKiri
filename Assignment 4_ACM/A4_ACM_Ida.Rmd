...
You have to design a study (aka plan the number of trials) assuming that people are using a reinforcement learning process to pick between 2 stimuli. In this study you expect there to be 2 conditions and the 1 participant playing the game will vary their learning rate between conditions. The difference in learning rate is .2: condition 1 has x - .1 and condition 2 x + .1, with x = 0.7. The temperature is the same: 0.5.
Identify a feasible number of trials and motivate it.


So condition 1 has x = 0.6, and condition 2 has x = 0.8.
...
# via dummy coding, tell stan that there are two conditions - and it should estimate two vlaues!
