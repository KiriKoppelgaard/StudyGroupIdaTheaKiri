...
for (sn in session_numbers){
  ...
  for (s in 1:sn){
    ...
    #define new priors
    prior_alphaMean1 <- log(mean(inv.logit(draws_df$alpha1)))
    prior_alphaSD1 <- log(sd(inv.logit(draws_df$alpha1)))
    prior_alphaMean2 <- log(mean(inv.logit(draws_df$alpha2)))
    prior_alphaSD2 <- log(sd(inv.logit(draws_df$alpha2)))
    prior_temperatureMean <- log(mean(inv.logit(draws_df$temperature)))
    prior_temperatureSD <- log(sd(inv.logit(draws_df$temperature)))
