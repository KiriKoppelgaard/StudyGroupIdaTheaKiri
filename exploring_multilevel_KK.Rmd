...
generated quantities{
  ...
  array[trials,agents] int<lower=0, upper = trials> prior_preds;   array[trials,agents] int<lower=0, upper = trials> posterior_preds; 
  ...
  beta_prior = inv_logit(normal_rng(betaM_prior, betaSD_prior));

  for (i in 1:agents) 
    prior_preds[,i] = binomial_rng(trials,     inv_logit(alpha_prior + to_vector(memory[,i]) * beta_prior)); 

  for (i n :agents) 
  posterior_preds[,i] = binomial_rng(trials, inv_logit(alpha[i] + to_vector(memoryLip * beta[i])); 
} 
