...
```{r}
data <- read.csv('bias_detector_agent_wsls_agent.csv')

data$Feedback_Other <- 1 - data$Feedback_Self[0:120]

#Recoding variables to stay and leave bias: 1 = heads, -1 = tails
data$StayBias[data$Feedback_Other == 1 & data$Other == 1] <- 1
data$StayBias[data$Feedback_Other == 1 & data$Other == 0] <- -1
data$StayBias[data$Feedback_Other == 0] <- 0

data$LeaveBias[data$Feedback_Other == 0 & data$Other == 1] <- -1
data$LeaveBias[data$Feedback_Other == 0 & data$Other == 0] <- 1
data$LeaveBias[data$Feedback_Other == 1] <- 0


data <- list(
  n = 120, 
  k = data$Other[0:120],
  f_stay = data$StayBias[0:120],
  f_leave = data$LeaveBias[0:120]
)
```
